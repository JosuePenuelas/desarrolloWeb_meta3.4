        ...
        <v-dialog v-model="showNewDialog" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field v-model="newTipo" label="Tipo"></v-text-field>
        ...
        <v-dialog v-model="showUpdateDialog" max-width="500px">
            <v-card>
                ...
                <v-card-text>
                    ...
                    <v-text-field v-model="updateTipo" label="Tipo"></v-text-field>
...
const headers = ref([
    ...
    { key: 'numInv', title: 'Número de inventario' },
    { key: 'tipo', title: 'Tipo' },
...
async function eliminar(id) {
    try {
        const response = await fetch(`https://localhost:4000/activos/id${id}`, {
            method: 'DELETE',
        });
        obtenerDatos();
...
async function actualizarItem() {
    try {
        const response = await fetch(`https://localhost:4000/activos/id${updateId.value}`, {
            method: 'PUT',
            body: JSON.stringify({
                id: updateId.value,
                numSerie: updateNumSerie.value,
                numInv: updateNumInv.value,
                tipo: updateTipo.value,
                desc: updateDesc.value,
                ubicacion: updateUbicacion.value,
...
async function cargarDatosUpdate() {
    try {
        ...
        updateNumInv.value = data.numInv;
        updateTipo.value = data.tipo;
        updateDesc.value = data.desc;
        updateUbicacion.value = data.ubicacion;
        updateResponsable.value = data.responsable;
