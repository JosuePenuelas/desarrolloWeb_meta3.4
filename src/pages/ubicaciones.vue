      ...
      <v-dialog v-model="showUpdateDialog" max-width="500px">
        <v-card>
          ...
          <v-card-text>
            ...
            <v-text-field v-model="updateActivosAsociados" label="Activos Asociados" @input="convertirLista"></v-text-field>
...
const headers = ref([
  ...
  { key: 'activosAsociados', title: 'Activos Asociados' },
  { key: 'imagenUbi', title: 'Imagen Ubicación' },
]);
...
async function eliminar(id) {
  try {
    const response = await fetch(`https://localhost:4000/ubicaciones/id${id}`, {
      method: 'DELETE',
    });
    obtenerDatos();
...
async function actualizarItem() {
  try {
    const response = await fetch(`https://localhost:4000/ubicaciones/id${updateId.value}`, {
      method: 'PUT',
      body: JSON.stringify({
        id: updateId.value,
        descripcion: updateDescripcion.value,
        activosAsociados: updateActivosAsociados.value,
        imagenUbi: updateImagenUbi.value,
      }),
      headers: {
        'Content-type': 'application/json; charset=UTF-8',
      },
